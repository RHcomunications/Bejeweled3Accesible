...
8. **Firma estable de Android (keystore commiteado)**:
 ...
 10. **Empaquetado compacto de `audio.pac` (162 MB → 14 MB) y orden de explosión en cascadas (v2026.08.28.2)**:
  ...
  15. **Locuciones Oficiales en Español Masterizadas con Acústica PopCap Arcade (v2026.09.01.0, 2026-09-01)**:
  ...
  16. **Hotfix: Remasterización de Locuciones en Español con FFmpeg e Integración de Selector Dual (v2026.09.01.1, 2026-09-01)**:
   ...
   20. **Filtrado Auténtico MO3 y Localización Dinámica en la Caja de Música / Jukebox (2026-09-15)**:
       - *Contexto:* La versión inicial de la Caja de Música exponía nombres crudos de archivos de disco (e.g. `03 - Classic Mode - Part 1`) e incluía temas no jugables o pistas medley/bonus de la carpeta de respaldo (`music/`).
       - *Solución:* 
    ...
    23. **Restauración Contextual Automática de Música en la Caja de Música / Jukebox (2026-09-15)**:
        - *Problema:* Al cambiar la Caja de Música en Opciones de vuelta al modo "Automático" (`"Auto"`), la música hacía fallback fijo al tema del Menú Principal (`MusicMap.MainTheme`), incluso si el usuario había entrado a Opciones desde el Menú de Pausa en medio de una partida (p. ej. en Modo Clásico, Relámpago, Zen, Póker, Mariposas, Tormenta de Hielo, Mina de Diamantes o Quest).
        - *Causa:* `CycleMusicBoxTrack` invocaba incondicionalmente `_sound.PlayMusic(Audio.MusicMap.FileName(Audio.MusicMap.MainTheme))` al volver a `curIdx == -1` ("Auto").
        - *Solución:* 
          1. Se implementó `PlayAutoMusicForCurrentContext()` y `PlayMusicForCurrentMode()`, apoyados en el nuevo helper canónico `MusicMap.GetModeMusicFileName(modeKey, level, questType)`.
          2. Al seleccionar "Automático", el sistema consulta el origen de la pantalla (`_optionsOriginScreen`): si se abrió desde el Menú de Pausa o partida activa, restaura inmediatamente la pista correspondiente al modo de juego actual (con su fase/nivel de Clásico, pista o ambiente de Zen, o variante específica de misión en Quest). Si se abrió desde el Menú Principal o pantallas de Relicarios de Quest, reproduce el tema adecuado (`MainTheme` o `QuestTheme`).
          3. Se amplió la suite de pruebas a **154 tests unitarios** verificando todas las transiciones y asignaciones por modo y tipo de misión.
