...
- **Principio rector — la música es clima / atmósfera / ambiente:** la música (módulo real `.mo3` y ambientales de Zen) se trata como **clima, atmósfera o acompañamiento ambiental del juego**, nunca como un objeto posicionado. Suena **espacializada como ambiente** (el `GridSpatializer` la trata con aire/profundidad y paneo simétrico L/R), envolvente y acompañante, nunca posicionada en el tablero. Las locuciones (voz del juego y lector de pantalla) también van **siempre centradas**, con *ducking* automático de la música mientras hablan. El motor espacial `GridSpatializer` procesa tanto los efectos de tablero como la música (esta última como ambiente, con L/R en fuentes simétricas).
...
- **Música real:** el módulo `Bejeweled3_suite.mo3` (62 minutos, extraído del propio juego) se decodifica con libopenmpt (BSD-3) y se reproduce en cadena saltando por los offsets del `music.xml` original, exactamente como el juego original (el intro avanza solo al menú a los 24 segundos). Se reproduce **espacializada como ambiente** (DSP de `GridSpatializer` con aire/profundidad y paneo simétrico L/R), como clima/atmósfera de acompañamiento envolvente (nunca posicionada en el tablero); véase el principio rector en §4.
