...
**Versión Actual:** Windows: `v2026.08.31.0` | Android: `android-v2026.08.27.2`  
**Tecnología Base:** 
...
8. **Firma estable de Android (keystore commiteado)**:
 ...
 10. **Empaquetado compacto de `audio.pac` (162 MB → 14 MB) y orden de explosión en cascadas (v2026.08.28.2)**:
  ...
  14. **Triple Auditoría: Optimización de Recursos del Sistema, Ciclo de Vida y Blindaje de Memoria (v2026.08.31.0, 2026-08-31)**:
      - *Contexto:* Tras tres pasadas exhaustivas de auditoría de accesibilidad, jugabilidad y manejo con el sistema operativo, se identificaron puntos críticos en la liberación de recursos no administrados, timers y pipeline de renderizado.
      - *Solución:* (a) Se encapsuló la lista sincronizada `_activeSpatialSweeps` y se blindó `SoundEngine.Dispose()` con detención, desasignación de pins `GCHandle` y liberación de handles de BASS síncrona para todos los timers en vuelo (`_musicDuckTimer`, `_musicMonitorTimer`, `_voicePumpTimer`, `_panSweepTimer`), previniendo cualquier acceso inválido tras `BASS_Free()`. (b) `MainWindow` y `ZenManager` implementaron llamadas exhaustivas a `Dispose()` en `OnFormClosing` para timers de formulario (`_lightningTimer`, `_loadingTimer`, `_renderTimer`, `_mantraTimer`, `_breathTimer`). (c) Se verificó la estabilidad total de anuncios de teclado y se preservó el 100% de la suite de tests unitarios (129/129).

---

## 🏆 7. Estado del Proyecto y Releases

- **Windows (`main`)**: **RAMA TERMINADA Y CONSOLIDADA AL 100%**. Release oficial `v2026.08.31.0` (Triple Auditoría completada: optimización de ciclo de vida de timers, liberación thread-safe de streams de BASS y barridos espaciales, eliminación de presión GDI, y blindaje de recursos). Suite de tests unitarios 100% en verde (129/129).
- **Android (`android`)**: **LUZ VERDE PARA CONTINUACIÓN Y DESARROLLO**. Se reactiva el foco de los contribuidores en el port oficial .NET 9 Android con TalkBack nativo, trasladando la madurez y ajustes de sonido/jugabilidad consolidados en Windows hacia el cliente móvil. Release base actual: `android-v2026.08.27.2`.
...
- **Flujo de release:** bump en `AssemblyInfo.cs`, `Localization.cs` (LoadingTitle/AppTitle) y `README.html` (versión + changelog ES/EN); en Windows build Debug+Release + suite completa (129/129) y zip con exe/PDB Release + `bass.dll` + `bass_fx.dll` (x64) + `bass_fx32.dll` (x86) + `nvdaControllerClient32.dll` + `libopenmpt.dll` + 4 `openmpt-*.dll` + `mscorlib.dll` + `norm*.nlp` + `es\` + `README.html` + `audio.pac` (generado por `--pack-audio`, ~14 MB) + `sounds\images\` completa (sin `sounds/*.ogg` ni `music/`). **Regla crítica del nombre del zip:** `Bejeweled3Accesible-<version>.zip` SIN ceros a la izquierda (p.ej. `2026.8.31.0`), porque `Version.ToString()` no rellena; si lleva ceros (`2026.08.31.0`) el updater arma otra URL y da 404. **NUNCA usar `/t:Rebuild`** (ver anecdotario 11). En Android el APK se compila en GitHub Actions (ver anecdotario 7); `gh release create` + `gh release upload`; limpiar `Temp\opencode`.
