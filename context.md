...
- **Escuela de Audio (calibración, anclada al tablero):** Desde el menú principal (`GameScreen.AudioSchool`) un mini menú reproduce pruebas por **columna** (azimut: A-H en la fila del fondo, misma profundidad), por **fila** (profundidad: 1-8 en la columna central), por **altura** (suelo/gema/aérea en el centro del fondo), un **barrido** de columnas y una demo **lejos con aire**, usando `SoundEngine.PlaySoundSpatialElevated` / `PlaySoundSpatialSweep` / `PlaySoundAtWorld`, que fuerzan el camino de objeto 3D. Verifica la configuración de auriculares sobre el propio tablero.
...
- **Git & Releases:** Tags `v2026.08.17.0`, `v2026.08.17.1`, `v2026.08.18.0` (audio y música reales), `v2026.08.18.1` (HRTF binaural paramétrico), `v2026.08.18.2` (hotfix: ruta binaural por DSP en el stream directo, select y combos audibles de nuevo), `v2026.08.19.0` (audio espacial de objeto estilo Dolby: sin pasos-bajo) `v2026.08.19.1` (hotfix: objeto puro, cero filtrado espectral) y `v2026.08.20.0` (audio espacial 3D por objetos Atmos + Escuela de Audio) publicados en GitHub con auto-actualizador integrado. El asset de cada release debe nombrarse `Bejeweled3Accesible-<versión sin ceros>.zip` (ver anécdota 6).
