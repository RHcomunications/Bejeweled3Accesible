...
**Versión Actual:** Windows: `v2026.08.25.1` | Android: `android-v2026.08.26.11`  
**Tecnología Base:** 
...
11. **Textos de interfaz coherentes con táctil en Android (override bilingüe en `Localization.cs`)**:
 ...
 15. **Release `android-v2026.08.26.4`: optimización de batería (cierre al día de la rama Android)**:
     - *Cambio:* Se publica como release firmada `android-v2026.08.26.4` (hotfix del mismo día: `ApplicationVersion` 11→12) el conjunto de optimizaciones de batería de la anécdota 14, más la limpieza de README y el audit táctil de las anécdotas 11–13. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` (LoadingTitle/AppTitle) y `README.html` (versión + changelog ES/EN). Windows queda sin tocar. Con esto la rama Android queda cerrada y al día; los cambios futuros de Android parten de este estado.

  16. **Release `android-v2026.08.26.5`: TalkBack vuelve a encontrar las celdas en apaisado**:
      - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 12→13) que corrige el bug de rotación a horizontal: TalkBack cacheaba los `Rect` de los 64 nodos virtuales del modo vertical y el toque no acertaba ninguna celda (fallaba el deslizar para mover; solo iba la navegación de dos dedos). En `GameScreenView.OnSizeChanged` y `MainActivity.OnConfigurationChanged` se envía un evento `WindowContentChanged` de accesibilidad (`Parent.RequestSendAccessibilityEvent`, patrón de `TalkBackBridge.cs`) para que TalkBack reconsulte las coordenadas. El intercambio por doble toque ya usaba índices de grilla, así que no cambió. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

  17. **Release `android-v2026.08.26.6`: doble swap accidental y barridos de audio dinámicos**:
      - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 13→14) que corrige dos hallazgos de revisión de código. (a) En `HandleBoardTouch`, tocar una celda contigua a la seleccionada (p. ej. tras Pista) y levantar el dedo con un leve arrastre >30 px disparaba un segundo intercambio no deseado, porque el `Up` recalculaba el origen desde la celda tocada (`_selectedX` ya estaba en `-1` tras el swap del `Down`). Se añade la bandera `_swapExecutedInCurrentTouch` que ignora el gesto de deslizamiento cuando el swap ya ocurrió en el `Down`. (b) En la Escuela de Audio, las opciones "Barrido izquierda→derecha" y "Barrido frente→fondo" ahora varían el paneo/profundidad de forma gradual (secuencia asíncrona con `Task.Run`+`Delay(120)`) en vez de un sonido fijo. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

  18. **Release `android-v2026.08.26.7`: TalkBack deja de pelear con los gestos (movimiento accesible real)**:
      - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 14→15) que cierra la experiencia de accesibilidad. (a) Modo inmersivo sticky en `MainActivity` (`EnableFullScreen` con `SystemUiFlags.ImmersiveSticky`, en `OnCreate`/`OnResume`/`OnConfigurationChanged`) oculta barra de estado y navegación. (b) `GameScreenView.DispatchHoverEvent` (`protected override`) consume `HoverMove`/`HoverEnter` y anuncia la gema bajo el dedo, habilitando exploración táctil con 1 dedo (antes TalkBack obligaba a 2 dedos). (c) Clave `HintTitle` localizada (botón "💡 Pista"). (d) Solución al conflicto swipe/TalkBack: cada gema del `GameAccessibilityNodeProvider` expone acciones de nodo direccionales (`ActionSwapUp/Down/Left/Right`, etiquetas `SwapUp/Down/Left/Right`), ejecutadas desde el menú de acciones de TalkBack y resueltas en `PerformAction` llamando `ExecuteSwap` (pasó a `internal`); el doble toque ya seleccionaba/intercambiaba vía `Action.Click`. El intercambio ya no depende de arrastrar. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

   20. **Release `android-v2026.08.26.9`: el "Welcome to Bejeweled" ya no suena en cada carga**:
       - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 16→17) que corrige un reporte de audio. El mapa de sonidos es idéntico al de Windows (mismo `AudioMap.cs` y mismos archivos `voice_welcomeback`/`voice_welcometobejeweled` cargados desde assets), así que el bug no estaba en el mapa sino en cuándo se disparaban las voces. En `NativeMenuManager`: `ShowLoadingScreen` ya no reproduce `voice_welcometobejeweled` en cada inicio; `ShowMainMenu(bool returningFromGame = false)` solo dice "Welcome to Bejeweled" cuando `Profiles.Count == 0` (primera ejecución + prompt de creación de perfil) y "Welcome back" solo cuando `returningFromGame == true`. `MainActivity.ReturnToMainMenu` llama `ShowMainMenu(true)` al volver de una partida, igual que Windows (`TransitionToMainMenu(speakWelcomeBack)`). La creación de perfil dentro de la partida (`GameScreenView.PromptCreateProfile`) también usa `voice_welcometobejeweled`. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

   21. **Release `android-v2026.08.26.10`: bienvenida solo al iniciar + audio espacial tipo Windows**:
       - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 17→18) que corrige un reporte de que "los sonidos no suenan". Auditoría: las 189 pistas de `AudioMap` ya estaban empaquetadas como `AndroidAsset` (`sounds\*.ogg`) y `GameScreenView` SÍ dispara los SFX del tablero (swap, cascadas, `combo_*`, gemas especiales y modos) vía `_sound.PlaySoundSpatial`, así que el problema no era el mapa ni la falta de disparadores. La diferencia real con Windows es el modelo espacial: Android usaba un paneo L/R burdo `(1±pan)` e ignoraba la profundidad de fila, mientras Windows renderiza por el `GridSpatializer` binaural (paneo equal-power + atenuación por profundidad). Se portó el modelo exacto a `AndroidSoundEngine`: `PlaySoundSpatial` ahora pasa `pan = SpatialAudio.PanColumn(col)` y `depth = SpatialAudio.DepthForRow(row)`, y `PlaySoundSpatialPan` aplica `lg=cos`, `rg=sin` y `VolumeForDepth(depth)` igual que `GridSpatializer`. Además se corrigió `ShowMainMenu` (parámetro `atStartup`): la bienvenida suena solo al arrancar la app, no al volver de una partida. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

   22. **Release `android-v2026.08.26.11`: binaural completo (modelo GridSpatializer de Windows vía PCM)**:
       - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 18→19) que portó el modelo espacial binaural COMPLETO de Windows al motor Android. Ahora `AndroidSoundEngine` decodifica cada OGG de SFX del tablero a PCM mono con `MediaExtractor`+`MediaCodec` (`DecodeMonoSamples`/`DownmixToMono`), aplica el `GridSpatializer` exacto (paneo equal-power + aire/low-pass por profundidad de fila + anchura estéreo por profundidad) y reproduce el PCM estéreo por `AudioTrack` (un `AudioTrack` STATIC por voz, liberado en `OnMarkerReached` vía `SpatialVoice`, con tope de 40 voces concurrentes y pre-decodificado en segundo plano de `gem_hit`/`combo_1..3`). Esto iguala a Windows: las filas traseras suenan más lejanas, opacas (aire cerrado) y más anchas. La diferencia clave respecto a `.10`: `.10` solo hacía paneo equal-power + volumen por profundidad vía SoundPool; `.11` añade el filtro de aire y el ensanchamiento estéreo. Red de seguridad: si la decodificación PCM falla en un dispositivo, `GetMonoSamples` marca el sonido en `_pcmFailed` y `PlaySoundSpatialPan` cae al modelo `.10` (SoundPool) sin que deje de sonar nada. Las voces no pasan por el binaural (siguen centradas/secas vía `PlaySound`). Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.

   19. **Release `android-v2026.08.26.8`: lectura limpia de cada gema con 1 dedo**:
       - *Cambio:* Hotfix del mismo día (`ApplicationVersion` 15→16) que afina la exploración táctil tras el análisis de Mafe. En `GameScreenView.CreateAccessibilityNodeInfo` cada nodo de gema ahora expone una etiqueta hablada concisa (`A1, Roja`) en `Text`, y `ContentDescription` conserva solo el detalle de movimientos válidos, de modo que TalkBack no repite el nombre de la gema dos veces. Se confirmó que las cajas de colisión ya usaban `GetLocationOnScreen` con la misma geometría que `OnDraw` (hueco de 4·density entre casillas), así que nunca se solapaban: lo que se percibía como "lee varias gemas a la vez" era el barrido natural al deslizar el dedo (una gema por celda), no un error de límites. Se mantuvo `DispatchHoverEvent` consumiendo el hover (un `AccessibilityNodeProvider` crudo no delega el hover al framework si se devuelve `base`), por lo que la exploración de 1 dedo sigue funcionando y con una sola habla por celda. Bumps en `csproj`, `AndroidAutoUpdater.CurrentVersion`, `Localization` y `README.html`. La rama Android queda definitivamente cerrada y al día.
...
- **Windows (`main`)**: Release `v2026.08.25.1` (hotfix: guarda de instancia única) con soporte completo de teclado, ratón hablado, audio binaural 3D, suite de 145 tests en verde y auto-actualizador multiplataforma (filtra tags `android-*`). Marcado como **Latest**.
    - **Android (`android`)**: Release `android-v2026.08.26.11` (hotfix: módulo MO3 real en el dispositivo, APK mucho más ligero, textos de interfaz coherentes con táctil, optimización de batería, TalkBack correcto en apaisado, sin doble swap accidental, movimiento accesible real con TalkBack, lectura limpia de cada gema con 1 dedo, "Welcome to Bejeweled" solo al iniciar, audio espacial tipo Windows y binaural completo vía PCM) con TalkBack 100% nativo (el tacto siempre activo), árbol de accesibilidad virtual de 64 nodos, reproductor de módulo `libopenmpt`→`AudioTrack` con el `.so` compilado en CI vía NDK (minimp3+stb_vorbis internos, sin dependencias externas) y los 29 MP3 eliminados del APK (baja de ~184 MB a pocos MB), auto-actualizador de APK que busca su propio tag `android-v…`, y APK firmado con keystore estable (actualización en sitio). El último hotfix `.11` añade el binaural completo de Windows al motor Android: decodifica cada OGG de SFX a PCM mono (MediaExtractor+MediaCodec), aplica el GridSpatializer exacto (aire/low-pass por profundidad + anchura estéreo) y reproduce por AudioTrack, con fallback automático al modelo .10 si la decodificación PCM falla en el dispositivo; el `.10` corrigió que la bienvenida suene solo al iniciar la app (no al volver de partida) y portó el paneo equal-power + profundidad; el `.9` corrigió que "Welcome to Bejeweled" no sonara en cada carga; el `.8` añadió etiqueta hablada concisa por gema (`A1, Roja`); el `.7` añadió modo inmersivo sticky, exploración con 1 dedo (`DispatchHoverEvent`), botón Pista localizado y acciones de nodo direccionales para mover gemas sin arrastrar (TalkBack ya no come los gestos); el `.6` corrigió el doble swap accidental y los barridos dinámicos de la Escuela de Audio; el `.5` arregló TalkBack en apaisado y el `.4` la optimización de batería.
